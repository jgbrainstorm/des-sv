...
def whiskerStat_firstcut(expid,plot=False):
    ...
    Note that here, the sigma is not fwhm. Sigma is given in arcsec
    """
    ff = gl.glob('/data/des08.b/data/jiangang/firstcut/iq_165131/DECam_00'+expid+'_??_cat.fits')
    ff.sort()
    data=[]
    goodext = np.append(np.arange(60),61)
    if len(ff) == 62:
...
def whiskerStat_firstcut_mike(expid):
    ...
    if len(ff) == 62:
        try:
            kwargs = {'root_dir' : '/data/des08.b/data/jiangang/firstcut/iq_165131','exp_num' : int(expid)}
