...
def analyze_r50_whisker():
    ...
    pl.subplot(4,1,1)
    pl.errorbar(np.arange(len(expid)),whk,whkerr,'bo')
    ...
    pl.ylim(0,0.5)
    pl.subplot(4,1,2)
    pl.errorbar(np.arange(len(expid)),r50,r50err,'bo')
    pl.grid()
    pl.ylabel('R50 (sextractor)')
    pl.hlines(0.522,-1,len(expid),color='green')
    pl.xticks(np.arange(len(expid)),np.repeat('',len(expid)))
    pl.subplot(4,1,3)
    pl.errorbar(np.arange(len(expid)),e1,e1err,'bo')
    pl.grid()
    pl.ylabel('e1 (weighted momts.')
    pl.ylim(-0.3,0.3)
    pl.xticks(np.arange(len(expid)),np.repeat('',len(expid)))
    pl.subplot(4,1,4)
    pl.errorbar(np.arange(len(expid)),e2,e2err,'bo')
    pl.grid()
